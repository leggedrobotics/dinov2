...
from dinov2.data import collate_data_and_cast_conv, MaskingGenerator, DataAugmentationDINODepthNormConv
...
def do_train(cfg, model, resume=False):
    ...
    optimizer = build_optimizer(cfg, model.get_params_student())
    (
        lr_schedule,
        wd_schedule,
        momentum_schedule,
    ...
    #     local_crops_size=cfg.crops.local_crops_size,
    # )
    data_transform = DataAugmentationDINODepthNormConv(
        cfg.crops.global_crops_scale,
        cfg.crops.local_crops_scale,
        cfg.crops.local_crops_number,
        global_crops_size=cfg.crops.global_crops_size,
    ...
    for data in metric_logger.log_every(
        ...
        if math.isnan(sum(loss_dict_reduced.values())):
            ...
            for key,val in loss_dict_reduced.items():
...
def main(args):
    ...
    model = SSLMetaArchDistillation(cfg).to(torch.device("cuda"))
